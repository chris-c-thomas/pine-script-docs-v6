...
**array_id (any array type)** Optional. The ID of an array to use as the new row. If the matrix is empty, the array can be of any size. Otherwise, its size must equal `matrix.columns(id)`. By default, the function inserts a row of [na](https://www.tradingview.com/pine-script-reference/v6/#var_na) values.
