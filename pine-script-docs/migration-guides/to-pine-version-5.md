---
title: "To Pine Script® version 5"
...
This guide documents the **changes** made to Pine Script from v4 to v5. It will guide you in the adaptation of existing Pine scripts to Pine Script v5. See our [Release notes](/pine-script-docs/release-notes/#october-2021) for a list of the **new** features in Pine Script v5.
...
![image](https://www.tradingview.com/pine-script-docs/_astro/v5_autocomplete.R-4HP09V_Z1R3GAs.webp)
...
```pine
//@version=4
study("Bollinger Bands", overlay = true) [middle, upper, lower] = bb(close, 5, 4)
...
```pine
//@version=5
indicator("Bollinger Bands", overlay = true) [middle, upper, lower] = ta.bb(close, 5, 4)
plot(middle, color=color.blue)
p1PlotID = plot(upper, color=color.green)
p2PlotID = plot(lower, color=color.green)
...
```// Will cause an error during conversion _integer = input.integer _bool = input.bool
