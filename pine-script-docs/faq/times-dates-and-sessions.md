---
title: "Times, dates, and sessions"
...
![image](https://www.tradingview.com/pine-script-docs/_astro/Times-dates-sessions-How-can-i-know-how-many-days-are-in-the-current-month-1.B9TubEEu_Z1VSzdL.webp)
...
![image](https://www.tradingview.com/pine-script-docs/_astro/Times-dates-sessions-How-can-i-get-bars-trading-date-on-overnight-symbols-1.BMFIXxaR_Z1zcsxU.webp)
...
![image](https://www.tradingview.com/pine-script-docs/_astro/Times-dates-sessions-How-can-i-track-highs-lows-for-a-specific-timeframe-1.CzQ5vZps_Vmwyy.webp)
...
![image](https://www.tradingview.com/pine-script-docs/_astro/Times-dates-sessions-How-can-i-track-the-highs-lows-within-a-specific-session-or-time-of-day-1.CKWpgvoE_i2qEW.webp)
...
Scripts can use custom time-based sessions by defining a session string, as explained in the [Time-based sessions](/pine-script-docs/concepts/sessions/#time-based-sessions) section of the [Sessions](/pine-script-docs/concepts/sessions) page.
...
- Finally, we pass the `hiLoBetweenTime()` function as the `expression` of the [request.security()](https://www.tradingview.com/pine-script-reference/v6/#fun_request.security) function, with the modified ticker as the `symbol`. The [request.security()](https://www.tradingview.com/pine-script-reference/v6/#fun_request.security) function evaluates the `hiLoBetweenTime()` function _within the context of the lower timeframe_ to calculate the session’s highs and lows.

![image](https://www.tradingview.com/pine-script-docs/_astro/Times-dates-sessions-How-can-i-track-highs-lows-between-specific-intrabar-hours-1.BEXeNwQA_2fjLL2.webp)
...
// Request data from lower timeframe using the `hiLoBetweenTime()` function. [highAtTime, lowAtTime] = request.security(ticker.modify(syminfo.tickerid, session.extended), lowerTfInput,
...
![image](https://www.tradingview.com/pine-script-docs/_astro/Times-dates-sessions-How-can-i-detect-a-specific-date-time-1.BRXAWfjw_Z26ERf4.webp)
...
![image](https://www.tradingview.com/pine-script-docs/_astro/Times-dates-sessions-How-can-i-know-the-date-when-the-highest-value-was-found-1.ScXUxlvD_Z13zsCr.webp)
...
- We declare the high and low labels with [var](/pine-script-docs/language/variable-declarations/#var) but supply [na](https://www.tradingview.com/pine-script-reference/v6/#var_na) values for the x and y coordinates, so that the labels do not appear until these parameters are defined.
...
Using the [time()](https://www.tradingview.com/pine-script-reference/v6/#fun_time) function with a [time-based session string](/pine-script-docs/concepts/sessions/#time-based-sessions) returns the time of the current bar if it is within the session time. If it’s outside the session time, the function returns [na](https://www.tradingview.com/pine-script-reference/v6/#var_na). We determine the first bar that falls within the session by verifying that the current returned time is not [na](https://www.tradingview.com/pine-script-reference/v6/#var_na) but was [na](https://www.tradingview.com/pine-script-reference/v6/#var_na) one bar earlier.
...
- To ensure that the numbers do not plot on top of one another, we use the technique described in the FAQ entry [How can I stack plotshape text?](/pine-script-docs/faq/strings-and-formatting/#how-can-i-stack-plotshape-text)

## Can I time the duration of a condition?

Normally, variables reset every time the script performs a new iteration. This process is called _rollback_ and is described in the [Execution model](/pine-script-docs/language/execution-model/#executions-on-realtime-bars) page of the User Manual.

By contrast, variables declared with [varip](https://www.tradingview.com/pine-script-reference/v6/#kw_varip) _preserve_ their values between realtime updates. This behavior allows scripts to track conditions during bars.

The following example script uses the `secondsSince()` function from the PineCoders’ [time](https://www.tradingview.com/script/tyeeNU9I-Time/) library. This function uses variables declared with [varip](https://www.tradingview.com/pine-script-reference/v6/#kw_varip) to track the number of seconds that elapse within a realtime bar while a condition remains true. Our script waits for price to move a user-defined number of ticks from the [open](https://www.tradingview.com/pine-script-reference/v6/#var_open), and then starts a timer. When the appropriate number of seconds elapses, it triggers an alert. A table displays these conditions in real time.

![image](https://www.tradingview.com/pine-script-docs/_astro/Times-dates-sessions-Can-i-time-the-duration-of-a-condition-1.BgWHsjTH_TIPMh.webp)
...
![image](https://www.tradingview.com/pine-script-docs/_astro/Times-dates-sessions-How-can-i-identify-the-nth-occurance-of-a-weekday-in-the-month-1.-p0cMTpr_1vjPjM.webp)
...
- We use an [enum input](/pine-script-docs/concepts/inputs/#enum-input) to specify the target weekday, and convert it to an integer using a [switch structure](/pine-script-docs/language/conditional-structures/#switch-structure).
...
![image](https://www.tradingview.com/pine-script-docs/_astro/Times-dates-sessions-How-can-i-implement-a-countdown-timer-1.f7Zg0trE_Z1bA1wA.webp)
