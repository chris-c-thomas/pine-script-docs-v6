...
Plots are _serial_ visuals that always return a result on each bar — although the result can be [na](https://www.tradingview.com/pine-script-reference/v6/#var_na). One plot therefore forms a _series_. By contrast, [drawing visuals](/pine-script-docs/visuals/overview/#drawing-visuals) instantiate _individual objects_. A single plot visual function call can display results on all the bars in the main series, no matter how many bars display in the series, while drawings adhere to a [drawing limit](/pine-script-docs/writing/limitations/#line-box-polyline-and-label-limits) of approximately \~500 objects.

A script creates plot visuals sequentially as it executes across the chart bars, so it cannot draw them into the past or future all at once like drawings. For example, `plot(close)` plots the _current_ [close](https://www.tradingview.com/pine-script-reference/v6/#var_close) on the _current_ bar. Pine’s [execution model](/pine-script-docs/language/execution-model/) then repeats this for every bar in the dataset.

Scripts create plots with [offsets](/pine-script-docs/visuals/plots/#offsets) in exactly the same way. They appear to end at past or future bars because the script executes the same plot call on each bar and simply displays each result the same _fixed_ number of bars forwards or backwards.
...
![image](https://www.tradingview.com/pine-script-docs/_astro/Visuals-overview-Plot-visuals-UI-integration-and-display-2.DvJhT7q-_11X6E2.webp)
...
Scripts can also use `display.*` arguments to show or hide [input](/pine-script-docs/concepts/inputs/) values next to the script title in the status line and Data Window.
...
Note that if the script generates any _dynamic colors_, the color pickers in the “Style” settings do not display. See the [Maintaining automatic color selectors](/pine-script-docs/visuals/colors/#maintaining-automatic-color-selectors) section of the [Colors](/pine-script-docs/visuals/colors/) page to learn more.

The `display.*` arguments represent the _default state_ of the script’s plot visuals. Disabling a plot from the indicator’s “Style” settings and then reactivating it causes the plot to revert to [display.all](https://www.tradingview.com/pine-script-reference/v6/#const_display.all), unless the indicator is reset to its default settings.
...
Each script instance can create a maximum of 64 plots. Depending on the complexity of the plot and its arguments, one function call can count _more than once_ towards the [plot count limit](/pine-script-docs/visuals/plots/#plot-count-limit). See the [plot limits](/pine-script-docs/writing/limitations/#plot-limits) section of the [Limitations](/pine-script-docs/writing/limitations/) page for more information.
...
Scripts can create and manage drawing visuals from _local_ scopes, so programmers can include drawing calls in [conditional structures](/pine-script-docs/language/conditional-structures/), [loops](/pine-script-docs/language/loops/), and [user-defined functions](/pine-script-docs/language/user-defined-functions/) or [methods](/pine-script-docs/language/methods/#user-defined-methods), unlike plot calls. While scripts _can_ call drawing functions globally, it’s rarely necessary to execute drawings on every bar. Further, because scripts that create drawing objects on each bar are likely to reach the [limit](/pine-script-docs/writing/limitations/#line-box-polyline-and-label-limits) for that drawing type, it’s more usual to create drawings in local scopes.
...
![image](https://www.tradingview.com/pine-script-docs/_astro/Visuals-overview-Drawing-visuals-2.BGEZYA-4_2p3Y7W.webp)
...
Most drawing types have x and y coordinates, so drawing objects move as the user scrolls the chart or zooms in or out. The only exception is _tables_, which are anchored to one of nine fixed positions in the pane itself. See the [Tables](/pine-script-docs/visuals/overview/#tables) section below for more details about their unique characteristics.
...
Both [plotshape()](https://www.tradingview.com/pine-script-reference/v6/#fun_plotshape) and [plotchar()](https://www.tradingview.com/pine-script-reference/v6/#fun_plotchar) have several `location` options, which can use either _relative_ or _absolute_ chart positions:
...
A [bgcolor()](https://www.tradingview.com/pine-script-reference/v6/#fun_bgcolor) call, like most visuals, affects the _script pane_ by default. It sets the background color behind the main bar series only when it’s overlayed in the main pane — when `overlay = true` for the script or `force_overlay = true` for [bgcolor()](https://www.tradingview.com/pine-script-reference/v6/#fun_bgcolor) — otherwise it sets the background for the equivalent space in a separate pane.

By contrast, the [barcolor()](https://www.tradingview.com/pine-script-reference/v6/#fun_barcolor) function **always** colors the main bar series in the _main_ pane, even when called by a script executing in a separate pane.
...
While a [line](https://www.tradingview.com/pine-script-reference/v6/#type_line) object can connect only two points with a straight line, a [polyline](/pine-script-docs/visuals/lines-and-boxes/#polylines) can connect _multiple_ points on the chart consecutively to create a straight or _curved_ line drawing. A polyline uses an [array](/pine-script-docs/language/arrays/) of [chart points](/pine-script-docs/language/type-system/#chart-points) to set the coordinates of its sequential line segments, which can contain up to 10,000 chart points.
...
Similar to other drawings, tables have various features that scripts can modify during execution using setter functions. These include _table-specific_ features like the frame, border, and height/width in the pane, as well as _cell-specific_ features like background color, alignment, and [text formatting](/pine-script-docs/visuals/text-and-shapes/#text-formatting).
...
![image](https://www.tradingview.com/pine-script-docs/_astro/Visuals-overview-When-to-use-Tables-3.d9t_lRT9_Z13VbCK.webp)
